...
\subsubsection{Ray Intersect Bounding Box}

Ray intersect with bounding box.

\subsection{Octree Partitioning}

In our pipeline, the input data is usually large. We might have millions of points and thousands of rays to compute intetrsections. If we iterate through all the points and rays, the computation will be very slow. Therefore, it's necessary to partition the data into smaller pieces, octree is one of the best options.


\subsubsection{Point Cloud}

We can directly use PCL's octree class to partition point cloud. Points are stored in leaf nodes with smaller bounding boxes. However, PCL's octree class do not offer any method to show the connections between nodes in different level. We have to build a complete tree with all needed information stored in each node.

Luckily, PCL provides methods to traverse octree either depth first or breadth first. We can use these methods to traverse the tree and get all the information we need step by step. 


\subsubsection{Mesh}

We still use PCL's octree class to partition mesh. But before that, we have to extract a point which can represent the triangle. We use the center of gravity of each triangle as the representative point.
